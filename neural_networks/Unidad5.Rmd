...
breast_factor <- breast[, ncol(breast)

```

Neural networks work best when the input data are scaled to a narrow range around zero. The solution is to rescale the data with a normalizing or standardization function
...
breast <- as.data.frame(lapply(breast[, -ncol(breast)], normalize))

breast %>% mutate_at(1:9, funs((.-min(.))/max(.-min(.))))
