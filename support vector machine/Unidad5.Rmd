...
# Predicci?n de c?ncer de mama (benigno/maligno) a partir de distintas variables biol?gicas
...
#p.train: 0.6666667
#  subtitulo: Predicci?n de la malignidad/benignidad de un c?ncer de mama
#  seed.train: 1234
#  seed.clsfier: 1234567
```
...
Como son pocas variables, se puede hacer una exploraci?n gr?fica mediante boxplot:

```{r frag3,echo=FALSE,fig.height=4}
summary(dataset)
boxplot(dataset[,1:9],main='Datos sin normalizar',col='brown',cex.axis=0.4,subset=dataset$Class)
abline(h=5,lwd=2)
```
...
#library(ggplot2)
plot <- ggplot(dataset, aes(x=Class, y=Bare.nuclei))
plot + geom_violin()
```
