...
```{r}
knn_isolet <- class::knn(train_data_coil, test_data_coil, train_label_coil, k=1, prob=TRUE)
prob <- attr(knn_isolet, "prob")
prob <- 2*ifelse(knn_isolet == "-1", 1-prob, prob) - 1

pred_knn <- prediction(prob, train_label_coil)
...
prob1 <- 2*ifelse(aminocoil_test_pred1 == "-1", 1-prob1, prob1) - 1
